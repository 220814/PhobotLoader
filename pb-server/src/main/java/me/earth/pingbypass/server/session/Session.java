...
public class Session extends Connection implements IConnection {
    ...
    private CommonListenerCookie cookie = CommonListenerCookie.createInitial(
            new GameProfile(UUID.randomUUID(), "Unknown"), 
            false
    );
    ...
    public void setupStatusHandler(PacketListener listener) {
        this.setupInboundProtocol(StatusProtocols.SERVERBOUND, (ServerStatusPacketListener) listener);
    }

    @SuppressWarnings("unchecked")
    public void setupConfigurationListener(PacketListener listener) {
        this.setupInboundProtocol(ConfigurationProtocols.SERVERBOUND, (ServerConfigurationPacketListener) listener);
    }

    @SuppressWarnings("unchecked")
    public void setupConfigWorldHandler(PacketListener listener) {
        this.setupInboundProtocol(GameProtocols.SERVERBOUND_TEMPLATE.bind(RegistryFriendlyByteBuf.decorator(server.getWorldStem().registries().compositeAccess())), (ServerGamePacketListener) listener);
    }

    @SuppressWarnings("unchecked")
    public void setupLoginHandler(PacketListener listener) {
        this.setupInboundProtocol(LoginProtocols.SERVERBOUND, (ServerLoginPacketListener) listener);
    }

    public void setupConfigurationFinished() {
    }
